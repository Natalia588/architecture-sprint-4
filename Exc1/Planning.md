...
Для скорейшего решения проблем с заказами (в ближайшие полгода):
  ...
  1. Сделать фильтр по запросам к системе. Для внутренних запросов ограничений нет. Для внешних запросов нужно ограничивать количество запросов с одного ip-адреса и от одного пользователя.
  2. Улучшение наблюдаемости - добавление в систему мониторинга, трассировки, логирования.
  3. Оптимизация кода, отвечающего за вычисление стоимости по 3D моделям. 
  4. Выделение функционала по расчету стоимости в отдельный сервис с возможностью масштабирования.
  5. Добавление клиентского уровня кэширования для MES UI
  6. Добавление серверного уровня кэширования в MES API
  7. Сохранение данных о внешних заказах в кэш и базу перед отправкой в Rabbit
  8. Добавление логики на обработку сбоев

Если можно взять только 3 задачи, то обязательно: 1, 2, 3.
